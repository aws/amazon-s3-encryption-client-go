...
//	handler, err := (kmsKeyHandler{apiClient: svc}).decryptHandler(ObjectMetadata{KeyringAlg: KMSKeyring, MatDesc: `{"kms_cmk_id":"test"}`})
//	if err != nil {
...
//	tConfig.EndpointResolverWithOptions = awstesting.TestEndpointResolver(ts.URL)
//	svc := kms.NewFromConfig(tConfig)
//	handler, err := newKMSKeyringEntryWithCMK(svc, "thisKey")(ObjectMetadata{KeyringAlg: KMSKeyring, MatDesc: `{"kms_cmk_id":"test"}`})
//	if err != nil {
//		t.Errorf("expected no error, but received %v", err)
//	}
//
